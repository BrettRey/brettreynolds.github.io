...
- **Valley Hunt outreach page deployed for the tree conjecture itself (2026-07-18).** Companion explainer/game at https://brettreynolds.ca/valley-hunt.html: exact BigInt independence-sequence engine, near-miss-ratio scoring per the paper's nm metric, challenges calibrated by fresh local computation (champion configs from Table "multiarm" reproduced exactly; Kadrawi n=26 LC-failure polys reproduced from results/analysis_n26.json byte-for-byte). All page claims audited against paper/main_v2.tex. Cross-linked with the First Descent page; publications.html tree entry now carries the companion link.
